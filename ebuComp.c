...
int main(int argc, char ** argv) { // main
    ...
    int check = checkErrorNoArr(imageData1, MAGIC_NUMBER_EBU);
    if (check == BAD_FILE) {
        return BAD_FILE;
    ...
    check = checkMallocAndData(imageData1, imageData1, "eu", 0);
    if (check == BAD_MALLOC) {
        return BAD_MALLOC;
    ...
    // Check for errors with the file and its format
    check = checkErrorNoArr(imageData2, MAGIC_NUMBER_EBU);
    if (check == BAD_FILE) {
        return BAD_FILE;
    } else if (check == BAD_MAGIC_NUMBER) {
        return BAD_MAGIC_NUMBER;
    ...
    check = checkMallocAndData(imageData2, imageData2, "eu", 0);
    if (check == BAD_MALLOC) {
        return BAD_MALLOC;
    } else if (check == BAD_DATA) {
        return BAD_DATA;
