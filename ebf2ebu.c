...
int main(int argc, char ** argv) { // main
    ...
    imageData -> f = fopen(argv[1], "r");
    imageData -> filename = argv[1];

    // Check for errors with the file and its format
    int check = checkErrorNoArr(imageData, MAGIC_NUMBER);
    if (check == 2) {
        return BAD_FILE;
    } else if (check == 3) {
    ...
    check = checkMallocAndData(imageData, imageData, "eb", 0);
    if (check == BAD_MALLOC) {
        return BAD_MALLOC;
