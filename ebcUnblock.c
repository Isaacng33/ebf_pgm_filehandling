...
#define MAGIC_NUMBER_EBCC 0x4345
...
int main(int argc, char ** argv) { // main
    ...
    int check = checkErrorNoArr(imageData, MAGIC_NUMBER_EBCC);
    if (check == BAD_FILE) {
        return BAD_FILE;
    ...
    imageData2 -> image = make2dArr(imageData -> height + 1, imageData -> width);
    check = checkMallocAndData(imageData, imageData2, "ec", 1);
    if (check == BAD_MALLOC) {
        return BAD_MALLOC;
    }
